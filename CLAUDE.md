...
- **過去バージョンのファイルを消さない。** URLは残す（ただし下記の通り最新版へ転送される）
- **公開中のバージョンにリダイレクトスタブを付けない。** `index.html` と相互に
  転送し合って無限ループになる。`tools/check.sh` が検出する
...
旧版を開くと `index.html` 経由で最新版へ転送される。ブックマークやタブの
再読み込みで古い版を遊び続けてしまうのを防ぐため。`tools/release.sh` が
`tools/redirect_stubs.py` を呼び、公開版以外の全ファイルの `<head>` 冒頭に
スタブを入れる（公開版からは必ず外す）。

転送先を「最新版のファイル名」ではなく `index.html` にしてあるので、
**新版を出しても旧ファイルを書き換え直す必要がない。**

セーブデータは `localStorage` のオリジン単位（キーは `labolic.*.pt1` で
バージョン非依存）なので、転送されても進行状況は引き継がれる。

**ロールバックでプレイテストしたいときは、そのファイルの
`<!-- LABOLIC-REDIRECT-START -->` から `END` までを手で削除する。**
`tools/check.sh` は NOTE を出すが通過する。

## 過去バージョンとリーダーボードについて

旧版は最新版へ転送されるため、通常の経路では旧版のスコアは記録されない。
ただし上記の手動ピン留めをした場合、その版のスコアも同じ Firebase
リーダーボード（`leaderboard.json` / `leaderboard_udm.json`）に記録される。
バランス調整を挟んだ場合、スコアの比較可能性が崩れる点に注意。
